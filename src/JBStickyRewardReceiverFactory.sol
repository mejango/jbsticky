...
contract JBStickyRewardReceiverFactory is IJBStickyRewardReceiverFactory {
    ...
    error JBStickyRewardReceiverFactory_InvalidGroupId(uint256 groupId);
    ...
    constructor(IJBStickyDistributor distributor) {
    ...
    function settleFor(address stickyToken, uint256 groupId, IERC20 token) external override returns (uint256 amount) {
        ...
        amount = JBStickyRewardReceiver(deployReceiverFor({stickyToken: stickyToken, groupId: groupId})).settle(token);
    ...
    function predictReceiverOf(address stickyToken, uint256 groupId) external view override returns (address receiver) {
        ...
        return Create2.computeAddress({
            ...
            bytecodeHash: keccak256(
                bytes.concat(type(JBStickyRewardReceiver).creationCode, abi.encode(DISTRIBUTOR, stickyToken, groupId))
            )
        });
    }
    ...
    function deployReceiverFor(address stickyToken, uint256 groupId) public override returns (address receiver) {
        ...
        receiver = address(
            new JBStickyRewardReceiver{salt: _saltOf({stickyToken: stickyToken, groupId: groupId})}({
