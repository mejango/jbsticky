...
function fixture() {
  ...
  const context = { source: route.source, destination: route.destination, receiver: A(5), groupId: 4000n, info: { stToken: A(6) }, owner: A(1), key: 'bridge-key' };
  ...
  const names = ['bridgeRecords', 'saveBridgeRecords', 'mutateBridgeRecords', 'canDiscardBridgeJournal', 'discardBridgeDraft', 'bridgeRouteId', 'reconcileBridgeRecord', 'prepareBridgeFunding', 'actOnBridgeMovement', 'decodeGroupId', 'groupLabel'];
  vm.runInContext(`const CRITERIA_BASE = 1000n;\n${names.map(functionSource).join('\n')}`, ctx);
