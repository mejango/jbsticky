...
3. A tenure group's window for a round is fixed by the round's start week and the group's `minWeeks`/`maxWeeks`. Its denominator equals the sum of the project's live in-window stake when the round is first funded, and no stake added afterwards can enter that window. A holder's allocation equals the pot times their live in-window stake at claim time over that denominator, capped by what the pot still holds, so the sum of claims never exceeds the pot.
