...
**`lockedUntil` overload.** `processSplitWith` (payout and reserved-token splits, `beneficiary` = the sticky token) reads criteria from the split's `lockedUntil` field: any value that decodes as a valid criteria `groupId` via `_isValidGroup` selects that window; `0` or any other value (including real lock timestamps) falls through to group 0.

This overload is collision-free by construction: core stores `lockedUntil` verbatim and the lock only engages when `block.timestamp < lockedUntil`, so every valid criteria encoding — a sparse subset of `[1000, 520520]` (only values matching `minWeeks * CRITERIA_BASE + maxWeeks` for a valid parameter pair) — is a 1970-era timestamp that never locks anything, while genuine lock timestamps are ≥ ~1.7e9 and route to group 0. The `[1000, 520520]` band is wider than the `[1, 520]` a bare `minWeeks`-only value would need, but `520520` seconds is still only ~6 days past the Unix epoch, so the whole band remains permanently inert against a real lock. The field only gains meaning when the split's hook is this distributor. Accepted trade: a split cannot be genuinely locked *and* criteria-carrying — locked splits fund the everyone-pool. A `lockedUntil` value below `CRITERIA_BASE` (e.g. `4`) decodes to `minWeeks == 0`, which `_isValidGroup` rejects, so it falls through to group 0 exactly like any other out-of-range value rather than reverting.

**Why not `context.groupId`.** The split hook context carries its own `groupId`, but it is JB's split-group lookup key, not a per-split payload: payout distributions query `splitsOf(projectId, rulesetId, uint256(uint160(token)))` and reserved-token distributions query `JBSplitGroupIds.RESERVED_TOKENS`, so the value is fixed by the distribution path and echoed back. A split group whose ID were set to a criteria value would never be looked up at all — the split would silently never distribute. `JBSplitGroupIds.RESERVED_TOKENS == 1` happens to fall inside the criteria band numerically, but it decodes to `minWeeks == 0`, which `_isValidGroup` rejects — so it isn't even a candidate collision under this encoding. The disqualifying problem is structural, not a numeric near-miss: a group ID is shared by every split in the group, which would rule out running (say) a 4-to-8-week cohort pot and an everyone-pool off the same payout token, since both splits would be forced to read the same `groupId`. `lockedUntil` is per-split and carries no meaning outside the encoding's sparse, always-past-timestamp range, so it is the field with room for this.
