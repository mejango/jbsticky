...
abstract contract StickyRealProjectLifecycle is StickyRealProjectFork {
    //*********************************************************************//
    ...
    function test_launchUsesPermanentV6ConfigurationAndProductionRewardSettings() public view {
        StickyDeployer deployer = StickyDeployer(_context.suite.deployer);
    ...
    function test_autoStickAfterFullExitRequiresTrustAndStartsFreshStreak() public {
        ...
        vm.expectPartialRevert(StickyAutoStick.StickyAutoStick_NotTrusted.selector);
    ...
    function test_autoStickApprovalFailureIsAtomicAndRetryCompoundsOnlyRewards() public {
        ...
        vm.expectPartialRevert(StickyAutoStick.StickyAutoStick_InsufficientAllowance.selector);
        // forge-lint: disable-next-line(unused-return)
        _autoStick.compoundFor({projectId: _projectId, holder: _alice, groupIds: _defaultGroup()});
        assertEq(_distributor.collectableFor(address(_token), uint256(uint160(_alice)), _rewardToken()), reward);
        assertEq(_context.underlying.balanceOf(_alice), walletBefore);
        ...
        // forge-lint: disable-next-line(unused-return)
        (AutoStickStatus status,,,) = _autoStick.statusOf(_projectId, _alice, _defaultGroup());
        ...
        StickyTranche[] memory tranches = _hook.tranchesOf(_projectId, _alice);
    ...
    function test_collectingVestedRewardsPreventsLaterCompoundFromSpendingWalletTokens() public {
        ...
        vm.expectPartialRevert(StickyAutoStick.StickyAutoStick_BelowMinimum.selector);
        // forge-lint: disable-next-line(unused-return)
        _autoStick.compoundFor({projectId: _projectId, holder: _alice, groupIds: _defaultGroup()});
        assertEq(_context.underlying.balanceOf(_alice), walletBefore + reward);
        _assertPosition(_alice);
    ...
    function test_grantsTrustRevocationAndSoulboundTransfersPreserveHolderConsent() public {
        ...
        vm.expectPartialRevert(StickyHook.StickyHook_SenderNotTrusted.selector);
        vm.prank(_bob);
        _context.core.terminal
            .previewPayFor({
                projectId: _projectId,
    ...
    function _assertPosition(address holder) internal view {
        StickyTranche[] memory tranches = _hook.tranchesOf(_projectId, holder);
        uint256 sum;
...
// forge-lint: disable-next-line(multi-contract-file)
contract StickyBase6ForkTest is StickyRealProjectLifecycle {
...
// forge-lint: disable-next-line(multi-contract-file)
contract StickyEthereum3ForkTest is StickyRealProjectLifecycle {
