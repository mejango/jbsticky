...
contract JBStickyDeployerRegressionTest is TestBaseWorkflow {
    ...
    function test_rejectsStickyTokenAsStakedToken() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                JBStickyDeployer.JBStickyDeployer_StakedTokenIsSticky.selector, address(shares), projectId
            )
        );
        vm.prank(_launcher);
    ...
    function test_rejectsTransferredProjectNft() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                JBStickyDeployer.JBStickyDeployer_UnexpectedNft.selector, address(jbProjects()), holder, projectId
            )
        );
        vm.prank(holder);
