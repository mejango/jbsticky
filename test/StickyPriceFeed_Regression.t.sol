...
import {StickyTestPriceFeed} from "./helpers/StickyTestPriceFeed.sol";

/// @notice Real core pricing registers and preserves Sticky's exact, immutable share-issuance denominator.
contract StickyPriceFeedRegressionTest is TestBaseWorkflow {
    //*********************************************************************//
    ...
    StickyPriceFeed internal _feed;
    ...
    StickyPricingToken internal _underlying;
    ...
    function setUp() public override {
        ...
        _underlying = new StickyPricingToken(6);
        ...
        _feed = StickyPriceFeed(address(_deployer.priceFeedOf(_projectId)));
    ...
    function test_constructorRejectsPriceRegistryMismatch() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                StickyDeployer.StickyDeployer_PriceRegistryMismatch.selector, address(jbPrices()), otherPrices
            )
        );
        new StickyDeployer({controller: jbController(), terminal: jbMultiTerminal()});
    }
    ...
    function test_defaultFallbackCannotChangeExactIssuance() public {
        ...
        // forge-lint: disable-next-line(literal-instead-of-constant)
        StickyTestPriceFeed wrongFeed = new StickyTestPriceFeed(5e6);
        ...
        vm.expectRevert(
            // forge-lint: disable-next-line(literal-instead-of-constant)
            abi.encodeWithSelector(StickyHook.StickyHook_UnexpectedIssuedCount.selector, _projectId, 5e18, 20e18)
        );
        // forge-lint: disable-next-line(literal-instead-of-constant)
        _stake(10e6);
    ...
    function test_feedPrecisionBoundaries() public {
        ...
        // forge-lint: disable-next-line(literal-instead-of-constant)
        vm.expectRevert(abi.encodeWithSelector(StickyPriceFeed.StickyPriceFeed_UnsupportedDecimals.selector, 37));
        // forge-lint: disable-next-line(literal-instead-of-constant,unused-return)
    ...
    function test_maxCurrencyUsesDistinctSyntheticPair() public {
        ...
        StickyPriceFeed feed = StickyPriceFeed(address(_deployer.priceFeedOf(projectId)));
        ...
        // forge-lint: disable-next-line(literal-instead-of-constant)
        StickyPricingToken(collisionToken).mint({account: _holder, amount: 12e6});
        vm.startPrank(_holder);
        // forge-lint: disable-next-line(literal-instead-of-constant,unused-return)
        StickyPricingToken(collisionToken).approve({spender: address(jbMultiTerminal()), value: 12e6});
    ...
    function test_zeroCurrencyIsRejectedBeforeProjectCreation() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(StickyDeployer.StickyDeployer_InvalidCurrency.selector, zeroCurrencyToken)
        );
