...
/// contracts.
// forge-lint: disable-next-line(multi-contract-file)
contract StickyRewardsRegressionTest is TestBaseWorkflow {
    //*********************************************************************//
    // ------------------------------ structs ---------------------------- //
    //*********************************************************************//
    ...
    function test_compoundKeepsZeroIssuanceRewardsClaimable() public {
        ...
        (AutoStickStatus status,,,) =
            fixture.adapter.statusOf({projectId: fixture.projectId, holder: _holder, groupIds: _defaultGroup()});
        assertEq(uint256(status), uint256(AutoStickStatus.ZeroIssuance));

        vm.expectRevert(
            abi.encodeWithSelector(
                // forge-lint: disable-next-line(literal-instead-of-constant)
                StickyAutoStick.StickyAutoStick_ZeroIssuance.selector,
                fixture.projectId,
                // forge-lint: disable-next-line(literal-instead-of-constant)
                999_999
            )
        ...
        returns (RewardFixture memory fixture)
    {
        ...
        fixture.distributor = new StickyDistributor({
        ...
        fixture.adapter = new StickyAutoStick({deployer: _deployer, distributor: fixture.distributor});
