...
import {StickyPricing} from "../src/libraries/StickyPricing.sol";
...
contract StickyPricingRegressionTest is TestBaseWorkflow {
    //*********************************************************************//
    ...
    function test_bootstrapBelowInitialMinimumPreviewsZeroAndRevertsAtomically() public {
        // forge-lint: disable-next-line(literal-instead-of-constant)
        _underlying = new StickyPricingToken(18);
        _projectId = _deploy({underlying: _underlying, tax: 0});
        // forge-lint: disable-next-line(literal-instead-of-constant)
    ...
    function test_bootstrapRoundingLossAboveOneBasisPointReverts() public {
        // forge-lint: disable-next-line(literal-instead-of-constant)
        _underlying = new StickyPricingToken(24);
        _projectId = _deploy({underlying: _underlying, tax: 0});
        // forge-lint: disable-next-line(literal-instead-of-constant)
        ...
        pure
    {
        ...
        uint256 weight =
            StickyPricing.weightFrom({amount: amount, supply: supply, backing: backing, decimals: decimals});
        if (weight == 0) return;
    ...
    function _assertZeroIssuanceRevertsAtomically(uint256 amount) internal {
        ...
        vm.expectRevert(abi.encodeWithSelector(StickyHook.StickyHook_ZeroIssuance.selector, _projectId, amount));
    ...
    function _deploy(StickyPricingToken underlying, uint256 tax) internal returns (uint256 projectId) {
    ...
    function _exerciseFirstMintBoundary(uint8 decimals) internal {
        _underlying = new StickyPricingToken(decimals);
        _projectId = _deploy({underlying: _underlying, tax: 0});
        // forge-lint: disable-next-line(literal-instead-of-constant)
