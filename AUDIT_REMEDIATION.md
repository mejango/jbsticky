...
This is a historical record of the revisions and validation runs named below. For current guarantees and remaining risks, use [INVARIANTS.md](./INVARIANTS.md) and [RISKS.md](./RISKS.md). Test counts and deployment predictions here do not describe a later compilation.

The 2026-09-12 review supersedes the global burn floor described in the September 8 addendum: it could trap a holder's remaining funds behind another holder's dust. Burns and cash outs now allow any remaining supply; the minimum applies only to initial issuance. Very small remaining supplies can make later deposits fail the unchanged rounding guard. `test_holderCanExitIndependentlyWhenAnotherHolderRetainsOneAtom` and `test_voluntaryBurnCanLeaveOneAtomAndUnsafeDepositsRevert` cover that boundary in `test/JBStickyPricing_Regression.t.sol`.

The current distributor pins only the current round when positive funding or settlement first records it; `poke()` also pins the following round if unset. Non-zero cash out tax raises the cost of snapshot capture but does not prevent it, and transferable shares can be returned without redeeming. Current user documentation states those limits and the exact cross-chain receiver prediction requirements.
...
| --- | --- | --- |
| C-01: zero transfers fabricate streaks; dust makes exits unbounded | Zero/self movements leave position accounting unchanged. Indexed cumulative tranche balances support logarithmic partial exits and constant-work full exits. Bounded pagination is available and used by the client. The tenure-rewards change (2026-09-24) changed the shape of this fix: exits now debit one epoch bucket per consumed tranche, and same-week joins merge into the newest tranche, so exit work is bounded by the distinct weeks consumed rather than constant. Same-week dust stays bounded (about 98,000 gas after 1,000 dust transfers); a ten-year weekly position exits in about 3.3M gas. | `JBStickyAccounting.t.sol`, `JBStickyHook_Unit.t.sol`, client action tests |
...
| --- | --- | --- |
| P-01 (High): a sole holder could bootstrap one share atom, or burn or cash out down to one, donate through `addToBalanceOf`, and the one-basis-point rounding guard then rejected every deposit below ten thousand atoms unless it was an exact multiple of the inflated atom price. A one-wei donation front-run broke exactness. The attacker's funds were fully recoverable on zero-tax projects. Earlier donation tests only exercised exact multiples. | Superseded by A-01 in [AUDIT_REPORT.md](./AUDIT_REPORT.md). The September 8 revision added a global burn floor; it made an honest holder's exit depend on another holder relinquishing dust, so it was removed. Burns and cash outs allow any remaining supply. `JBStickyPricing` keeps a `1e12`-atom minimum for bootstrap issuance only, and the one-basis-point rounding guard is unchanged. The burn-to-one-atom deposit lockout is an accepted risk described in [RISKS.md](./RISKS.md): very small supplies with donated backing can make inexact deposits preview zero and revert, rejected payments take no funds, and every holder can still exit independently. | `test_voluntaryBurnCanLeaveOneAtomAndUnsafeDepositsRevert`, `test_holderCanExitIndependentlyWhenAnotherHolderRetainsOneAtom`, `test_bootstrapBelowInitialMinimumPreviewsZeroAndRevertsAtomically` in `test/JBStickyPricing_Regression.t.sol` |
